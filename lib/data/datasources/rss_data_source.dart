...
abstract class RssDataSource {
  Future<List<RssItemModel>> fetchFeed(String url);
}

class RssDataSourceImpl implements RssDataSource {
  final http.Client _client;

  RssDataSourceImpl({http.Client? client}) : _client = client ?? http.Client();
  ...
  Future<List<RssItemModel>> fetchFeed(String url) async {
    try {
      ...
      ).timeout(const Duration(seconds: AppConstants.rssTimeoutSeconds));
      ...
      return _parseXml(xmlBody, url);
    } catch (e) {
      throw Exception('Network or parsing error while fetching RSS: $e');
    }
  }

  List<RssItemModel> _parseXml(String xmlBody, String feedUrl) {
    ...
    for (final element in itemElements) {
      ...
      final id = guid.isNotEmpty
          ? guid
          : (link.isNotEmpty ? link : '${feedUrl}_$index');
      ...
      items.add(
        RssItemModel(
          ...
          pubDate: pubDate,
          source: source,
          link: link.isNotEmpty ? link : feedUrl,
        ),
      );
      index++;
    }
